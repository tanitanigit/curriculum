...
class Person {
	...
	// 戻り値なしのメソッド「buy」を、引数としてCarクラスの「car」を代入して定義
	public void buy(Car car) {
		// carのメソッド「setOwner」を呼び出し、戻り値としてこのクラス内の「fullname」を指定
		car.setOwner(this.fullName());
		// 指定の文字列を表示
		System.out.println(car.getOwner() + "が購入しました");
	}

	// 戻り値なしのメソッド「buy」を、引数としてBicycleクラスの「bicycle」を代入して定義
	public void buy(Bicycle bicycle) {
		// bicycleのメソッド「setOwner」を呼び出し、戻り値としてこのクラス内の「fullName」を指定
